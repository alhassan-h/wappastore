...
class CustomerController extends Controller
{
    ...
    public function cart(Request $request)
    {
        ...
        $cart = ($customer->cart)?$customer->cart->sortByDesc('created_at'):[];
    ...
    /**
     * add a product to cart from store.
     *
     * @return;
     */
    public function storeAddToCart(Request $request)
    {
        ...
        if( $available_quantity < ($quantity * 12) ){
            return redirect()->back()->with('error', 'Product quantity insufficient!');
        }

        if( $customer->hasInCart($product, 'bulk') ){
            return redirect()->back()->with('warning', 'Product already in cart!');
        }else{
            $new_quantity = $available_quantity - ($quantity * 12);
            $product->update(['quantity' => $new_quantity]);
    
            $validatedData['customer_id'] = $customer->id;
            $validatedData['type'] = 'bulk';
            $validatedData['quantity'] = $quantity;
    
            $cart = Cart::create($validatedData);
        }

        return redirect()->back()->with('success', 'Product bulk added to cart successfully!');
     ...
     * @return;
     */
    public function storeUpdateCart(Request $request)
    {
        $user = $request->user();
        ...
        if( $available_quantity < ($diff * 12)){
            return redirect()->back()->with('error', 'Product quantity insufficient!');
        }
        
        $new_quantity = $available_quantity - ($diff * 12);
    ...
    public function deleteCart(Request $request)
    {
        ...
        $cart_product = $user->customer->cart->where('id', $cart_product_id)->first();
        $cart_product_quantity = ($cart_product->hasDiscount())?$cart_product->quantity * 12:$cart_product->quantity;
        $product = $cart_product->product;

        $product->increment('quantity', $cart_product_quantity);
        $cart_product->delete();
    ...
    public function checkout(Request $request)
    {
        ...
        foreach( $cart_products as $cart_product ){
            ...
            $validatedData['customer_id'] = $customer->id;
            $bulk = intval($cart_product->quantity);
            $paid_price = $bulk * $cart_product->getDiscountPrice();
            $validatedData['quantity'] = $bulk;
